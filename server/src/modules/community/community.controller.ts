...
export class CommunityController {
  ...
  async createComment(@Param('postId') postId: number, @Body() createCommentDto: CreateCommentDto, @Request() req: UserRequest): Promise<void> {
    const userId = req.user.id;
    return await this.communityService.createComment(postId, createCommentDto, userId);
  }

  // 댓글 수정
  @UseGuards(Jwt)
  @Put('articles/:postId/comments/:commentId')
  ...
  // 대댓글 생성
  @UseGuards(Jwt)
  @Post('articles/:postId/comments/:parentId/replies')
  ...
  async createReply(@Param('postId') postId: number, @Param('parentId') parentId: number, @Body() createCommentDto: CreateCommentDto, @Request() req: UserRequest): Promise<void> {
    const userId = req.user.id;
    return await this.communityService.createReply(parentId, createCommentDto, userId);
  }

  // 대댓글 수정
  @UseGuards(Jwt)
  @Put('articles/:postId/comments/:commentId/replies')
  ...
  // 대댓글 삭제
  @UseGuards(Jwt)
  @Delete('articles/:postId/comments/:commentId/replies')
  ...
  // 상위 댓글 조회 (페이징 지원)
  @Get('articles/:postId/comments/top-n')
  ...
  // 대댓글 조회 (페이징 지원)
  @Get('articles/:postId/comments/:commentId/replies')
  ...
  @ApiResponse({ status: 404, description: '상위 댓글을 찾을 수 없습니다.' })
  async getReplies(@Param('postId') postId: number, @Param('commentId') commentId: number, @Query() paginationQuery: PaginationQueryDto): Promise<{ replies: CommentResponseDto[]; total: number }> {
    // 상위 댓글 존재 여부 확인
    ...
    const [replies, total] = await this.communityService.getReplies(postId, commentId, paginationQuery);
