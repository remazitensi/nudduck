      ...
      <main className='mx-auto max-w-6xl p-4'>
        ...
        <section className='my-6 flex justify-center'>
          ...
          >
            ...
            <div className="absolute left-[31px] top-1/2 h-[29px] w-[175px] -translate-y-1/2 transform text-center font-['Pretendard'] text-3xl font-semibold text-[#909700]">
              {isQuoteVisible ? '오늘의 명언' : '오늘의 영문장'}
            </div>
            <div className="absolute left-[222px] top-[50px] text-left font-['Pretendard'] text-xl font-normal text-[#313119]">
              {isQuoteVisible && quote ? (
                <>
                  <div className='text-xl'>"{quote.message}"</div>
                ...
                englishSentence && (
                  <>
                    <div className='text-xl'>{englishSentence.korean}</div>
                    <div className='text-base text-gray-500'>({englishSentence.english})</div>
        ...
        {!isLoggedIn && (
          <section className='my-6 flex items-center justify-between rounded p-6'>
            ...
            <div className='ml-4 flex flex-col'>
              <div className='relative rounded-[10px] bg-[#A1DFFF] p-[20px] shadow-xl transition-transform duration-300 hover:scale-105'>
        ...
        <section className='mx-auto my-6 max-w-6xl p-6'>
          ...
          <div className='mt-4 grid grid-cols-1 gap-6 md:grid-cols-4 lg:grid-cols-4'>
            ...
            <div className='relative h-[260px] w-full rounded-lg border border-gray-300 bg-[#fbfaec] p-4 transition-all duration-300 hover:border-[#AEAC9A] hover:shadow-lg'>
              ...
              <div className='absolute left-[15px] top-[100px] mr-[4px] text-lg font-bold leading-[25px] text-black'>AI와의 가상 면접을 체험해 보세요!</div>
              <button
            ...
            {/* 두 번째 카드 */}
            <div className='relative h-[260px] w-full rounded-lg border border-gray-300 bg-white p-4 transition-all duration-300 hover:border-[#AEAC9A] hover:shadow-lg'>
          ...
          <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3'>
            {posts.length > 0 ? (
              posts.map((post, index) => {
                ...
                // 현재 날짜와 게시글의 작성 날짜를 비교
                const postDate = new Date(post.createdAt);
                const currentDate = new Date();
                const isToday = postDate.toDateString() === currentDate.toDateString();
                const isValidDate = !isNaN(postDate.getTime());

                // 조회수가 가장 높은 게시글 확인
                const isHighestView = index === 0;

                return (
                  <div
                    key={post.postId}
                    className='flex h-[160px] cursor-pointer flex-col justify-between rounded-lg border border-gray-300 bg-white p-4 transition-all duration-300 hover:border-[#AEAC9A] hover:shadow-lg'
                  ...
                  >
                    ...
                    <div className='flex items-center'>
                      <h3 className='mb-2 text-lg font-bold text-black'>
                        {post.title}
                        {/* NEW 또는 HOT 레이블을 제목 끝에 배치 */}
                        {isToday && <span className="ml-2 inline-block align-middle font-['Pretendard'] text-[14px] font-medium leading-tight text-[#A1DFFF]">NEW!</span>}
                        {isHighestView && <span className="ml-2 inline-block align-middle font-['Pretendard'] text-[14px] font-medium leading-tight text-[#FFC5C3]">HOT!</span>}
                      </h3>
                    </div>
